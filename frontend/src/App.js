...
const API_BASE_URL = process.env.REACT_APP_API_URL || `${window.location.protocol}//localhost:5000`;
...
function App() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/start-monitoring`, { method: 'POST' })
  ...
  const fetchAllData = async () => {
    try {
      const [statsRes, filesRes, logsRes, procRes, backupsRes] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}/api/defense-logs`),
        fetch(`${API_BASE_URL}/api/suspicious-processes`),
        fetch(`${API_BASE_URL}/api/backups`)
      ]);
  ...
  const handleSimulateAttack = async (mode = 'mass') => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/simulate-attack`, {
        method: 'POST',
  ...
  const handleDecryptAttack = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/decrypt-attack`, { method: 'POST' });
      const data = await res.json();
  ...
  const handleCreateBackup = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/create-backup`, { method: 'POST' });
      const data = await res.json();
  ...
  const handleRestoreBackup = async (timestamp) => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/restore-backup/${timestamp}`, {
        method: 'POST'
      });
      const data = await res.json();
      if (data.status === 'success') {
  ...
  const handleToggleAutoRemediation = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/toggle-auto-remediation`, { method: 'POST' });
      const data = await res.json();
  ...
  const handleResetEnv = async () => {
    try {
      await fetch(`${API_BASE_URL}/api/reset-environment`, { method: 'POST' });
  ...
  const handleCreateFile = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/create-file`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleEncryptSingleFile = async (filename) => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/encrypt-file/${filename}`, { method: 'POST' });
      const data = await res.json();
      showToast(`🔒 ${data.message}`, 'warning');
  ...
  const handleDeleteSingleFile = async (filename) => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/delete-file/${filename}`, { method: 'DELETE' });
      const data = await res.json();
  ...
  const handleFetchForensicReport = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/forensic-report`);
      const data = await res.json();
